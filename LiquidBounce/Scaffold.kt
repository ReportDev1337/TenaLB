...
class Scaffold : Module() {
    ...
    // Visuals
    private val counterDisplayValue = ListValue("Counter", arrayOf("Off", "Simple","Simple2", "Advanced", "Sigma", "Novoline", "Exhibition"), "Simple")
    private val modeDisplay = BoolValue("ModeDisplay", true)
    private val render = ListValue("Render", arrayOf("None", "Mark", "DrawCircle"), "DrawCircle")
    ...
    override fun onEnable() {
        ...
        if (autoDisableSpeedValue.get() && LiquidBounce.moduleManager.getModule(
            ...
            if (hudmod.NotifyForAllModule.get()) {
                ...
                NotificationManager.addNotification("Speed is Disabled To Prevent Flags-Errors.", Notification.Type.WARNING)
            }
        }
    ...
    fun onRender3D(event: Render3DEvent?) {
        ...
        val timer = mc.timer
        if (render.get().equals("Mark", ignoreCase = true)) {
        ...
        if (render.get().equals("DrawCircle", ignoreCase = true)) {
            ...
            RenderUtils.shadow(mc.thePlayer, posX, posY, posZ, 1.0, 64, Color(255,255,255,255).rgb)
            RenderUtils.cylinder(mc.thePlayer, posX, posY, posZ, 1.0, 64, Color(255,255,255,255).rgb)
        }
    }
