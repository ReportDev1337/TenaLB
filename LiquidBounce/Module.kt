...
import net.ccbluex.liquidbounce.features.module.modules.render.AutoDisable.DisableEvent
...
import net.ccbluex.liquidbounce.utils.flux.AnimationHelper
...
open class Module : MinecraftInstance(), Listenable {
    ...
    val colorlessTagName: String
        get() = "$name${if (tag == null) "" else " " + ColorUtils.stripColor(tag)}"
        ...
        set(value) {
            ...
            if (!LiquidBounce.isStarting && !forceNoSound) {
                when (LiquidBounce.moduleManager.toggleSoundMode) {
                    1 -> mc.soundHandler.playSound(PositionedSoundRecord.create(ResourceLocation("random.click"),1F))
                    2 -> (if (value) LiquidBounce.tipSoundManager.enableSound1 else LiquidBounce.tipSoundManager.disableSound1).asyncPlay(LiquidBounce.moduleManager.toggleVolume)
                    3 -> (if (value) LiquidBounce.tipSoundManager.enableSound2 else LiquidBounce.tipSoundManager.disableSound2).asyncPlay(LiquidBounce.moduleManager.toggleVolume)
                    4 -> (if (value) LiquidBounce.tipSoundManager.enableSound3 else LiquidBounce.tipSoundManager.disableSound3).asyncPlay(LiquidBounce.moduleManager.toggleVolume)
                    5 -> (if (value) LiquidBounce.tipSoundManager.enableSound4 else LiquidBounce.tipSoundManager.disableSound4).asyncPlay(LiquidBounce.moduleManager.toggleVolume)
                }
                if (LiquidBounce.moduleManager.shouldNotify)
                NotificationManager.addNotification(if (value) "$name Enabled" else "$name Disabled", if (value) Notification.Type.SUCCESS else Notification.Type.ERROR)
            }
...
*/
     ...
     * Print [msg] to chat
     */
    protected fun chat(msg: String) = ClientUtils.displayChatMessage("§4[§6§lKyino§7Sense§4] §r$msg")
